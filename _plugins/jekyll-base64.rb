...
module Jekyll
    class Base64Encoding < Liquid::Tag
        ...
        def render(context)
            ...
            # if a relative url was defined, the basepath is the same
            # as the page in which the image was requested.
            if (@imgsrc.chars.first != "/")
                if (@cs[:page]["dir"])
                  basepath = File.join(basepath, @cs[:page]["dir"])
                elsif (@cs[:page]["relative_path"]) # custom collections don't have a page-dir
                  basepath = File.join(basepath, @cs[:page]["relative_path"])
                end
                @abspath = Pathname.new(File.expand_path(@imgsrc, basepath))  # goes up levels in relative path
            else
