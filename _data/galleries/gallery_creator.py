...
# notes:
# original filename shouldn't contain a dash (-)
# file extension renamed to lower case
...
# set correct path
path = join("../../assets/img-noresample/",
            image_path)  # opieters used "../../assets/photography/" # customized Egbert for iMac 2024
...
for f in files:
    if f[f.rfind('-') + 1:f.rfind('.')] != 'thumbnail':
        newf = f[:f.rfind('-')] + "-%sx%s" % imagesize.get(join(path, f)) + f[f.rfind('.'):].lower()
...
with ExifToolHelper() as eth:
    for pic in new_gallery:
        ...
        if not found:
            ...
            if originals[pic][originals[pic].rfind('.') + 1:] in ['jpg', 'jpeg', 'png', 'PNG']:
                ...
                old_gallery.append(
                    {"filename": pic, "sizes": new_gallery[pic], "thumbnail": thumbs[pic], "original": originals[pic],
